...
const atmosphere: Record<Screen, { background: string; overlay: string; label: string; feeling: string }> = {
  portal: { background: IMG.portalAlt, overlay: "bg-black/52", label: "Arrival Gate", feeling: "arriving at the farm" },
...
function StatusCard({ eyebrow, title, detail }: { eyebrow: string; title: string; detail: string }) {
  return <div className="rounded-2xl border border-white/10 bg-white/10 p-4 backdrop-blur-xl"><div className="text-xs uppercase tracking-[0.25em] text-emerald-100/70">{eyebrow}</div><div className="mt-3 text-lg font-black">{title}</div><div className="mt-2 text-sm text-emerald-50/75">{detail}</div></div>;
}
...
function PhotoCard({ title, subtitle, image, height = "320px", children }: { title: string; subtitle?: string; image: string; height?: string; children?: React.ReactNode }) {
  return (
    <div className="group relative overflow-hidden rounded-[2rem] border border-white/10 bg-black shadow-[0_30px_80px_rgba(0,0,0,.55)] transition duration-700 hover:scale-[1.01]" style={{ minHeight: height }}>
      ...
      <div className="relative z-10 flex min-h-full flex-col justify-end p-6"><div className="text-3xl font-black leading-tight drop-shadow-2xl">{title}</div>{subtitle && <div className="mt-3 max-w-3xl text-sm leading-6 text-emerald-50/85">{subtitle}</div>}{children && <div className="mt-5">{children}</div>}</div>
    </div>
  );
}
...
function MetricTile({ label, value, note }: { label: string; value: string; note: string }) {
  return <div className="rounded-2xl border border-white/10 bg-white/10 p-5 backdrop-blur-xl"><div className="text-xs uppercase tracking-[0.25em] text-emerald-100/70">{label}</div><div className="mt-3 text-2xl font-black">{value}</div><div className="mt-2 text-sm leading-6 text-emerald-50/75">{note}</div></div>;
}
  ...
  return (
    <Shell screen="portal" setScreen={setScreen}>
      <section className="grid min-h-[86vh] items-center gap-10 lg:grid-cols-[1fr_.95fr]">
        <div><div className="text-xs uppercase tracking-[0.45em] text-emerald-100/80">Bronson Family Farm Ecosystem</div><h1 className="mt-6 max-w-4xl text-5xl font-black leading-[0.92] tracking-tight md:text-7xl">Enter The Ecosystem</h1><p className="mt-8 max-w-3xl text-xl leading-10 text-emerald-50/90">Every participant enters first as a guest and chooses how they want to experience the farm.</p><div className="mt-10 flex flex-wrap gap-4"><PillButton variant="light" onClick={() => setScreen("home")}>Continue As Guest</PillButton><PillButton onClick={() => setScreen("pathways")}>Create Account</PillButton><PillButton onClick={() => setScreen("pathways")}>Login</PillButton></div></div>
...
function Pathways({ setScreen }: { setScreen: (screen: Screen) => void }) {
  ...
  return <Shell screen="pathways" setScreen={setScreen}><GlassPanel eyebrow="Living Crossroads" title="Choose how you want to move through the ecosystem."><p className="max-w-4xl text-lg leading-9 text-emerald-50/85">This is not a menu. It is the center of the farm. Each pathway feels like a different place on the same land.</p></GlassPanel><div className="mt-6 grid gap-6 md:grid-cols-2 xl:grid-cols-3">{pathways.map((path) => <PhotoCard key={path.title} title={path.title} subtitle={path.subtitle} image={path.image}><PillButton variant="light" onClick={() => setScreen(path.screen)}>Enter</PillButton></PhotoCard>)}</div></Shell>;
}

function Guest({ setScreen }: { setScreen: (screen: Screen) => void }) {
  return <Shell screen="guest" setScreen={setScreen}><section className="grid gap-6 lg:grid-cols-[1fr_1.1fr]"><GlassPanel eyebrow="Guest Experience" title="Wonder first. Understanding second."><p className="text-lg leading-9 text-emerald-50/85">Guests experience the farm as an invitation. They can explore the story, view pathways, see today’s activity, and decide whether to participate more deeply.</p><div className="mt-8 flex flex-wrap gap-4"><PillButton variant="light" onClick={() => setScreen("pathways")}>Explore Pathways</PillButton><PillButton onClick={() => setScreen("operations")}>See Today</PillButton><PillButton onClick={() => setScreen("marketplace")}>Visit Marketplace</PillButton></div></GlassPanel><div className="grid gap-6"><PhotoCard title="Look Around" subtitle="The farm explains itself through people, land, food, work, and community." image={IMG.portal} /><div className="grid gap-4 md:grid-cols-3"><MetricTile label="No Login" value="Guest" note="Explore freely." /><MetricTile label="Invitation" value="Open" note="Choose when ready." /><MetricTile label="Next Step" value="Role" note="Participate deeper." /></div></div></section></Shell>;
}
