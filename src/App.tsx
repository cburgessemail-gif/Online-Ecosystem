...
function bg(image: string) {
  return {
    backgroundImage: `linear-gradient(to top, rgba(0,0,0,.88), rgba(0,0,0,.30), rgba(0,0,0,.08)), url("${image}")`,
...
function Navigation({
  ...
  return (
    <div className="mb-8 flex flex-wrap gap-3">
      <PillButton active={screen === "home"} onClick={() => setScreen("home")}>
        Entrance
      </PillButton>
      <PillButton active={screen === "roles"} onClick={() => setScreen("roles")}>
        Role Pathways
      </PillButton>
      <PillButton active={screen === "marketplace"} onClick={() => setScreen("marketplace")}>
        Marketplace
      </PillButton>
      <PillButton active={screen === "events"} onClick={() => setScreen("events")}>
        Events
      </PillButton>
      <PillButton active={screen === "nutrition"} onClick={() => setScreen("nutrition")}>
        Nutrition
      </PillButton>
    </div>
  );
    ...
    <Shell screen="home" setScreen={setScreen}>
      ...
      <section className="mt-24">
        ...
        <p className="mt-8 max-w-5xl text-xl leading-10 text-emerald-50/85">
          ...
          ecosystem supports marketplaces, schools, wellness initiatives,
        ...
        <Card className="overflow-hidden">
          ...
          <div className="p-8">
            ...
            <p className="mt-6 text-lg leading-9 text-emerald-50/82">
              Youth cultivate, harvest, prepare, organize, reflect, lead, and
              help move food toward marketplaces, schools, community
...
export default function App() {
  ...
  if (screen === "events") {
    return (
      <Placeholder title="Events" screen="events" setScreen={setScreen} />
    );
  }
