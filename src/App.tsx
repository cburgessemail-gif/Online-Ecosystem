...
const ui = {
  English: {
    ...
    title: "Summer 2026 Digital Training & Supervisor Management System",
  ...
  Español: {
    ...
    subtitle: "Sistema digital guiado para jóvenes, supervisores, padres, socios y resultados del programa.",
  ...
  Tagalog: {
    ...
    subtitle: "Digital training at supervision system para sa kabataan, supervisors, pamilya, at partners.",
    guided: "Simulan Tour",
  ...
  Italiano: {
    ...
    hero: "Benvenuti",
    subtitle: "Sistema digitale guidato per giovani lavoratori, supervisori, famiglie e partner.",
    guided: "Inizia Tour",
  ...
  עברית: {
    ...
    hero: "ברוכים הבאים",
    subtitle: "מערכת דיגיטלית לנוער, מפקחים, משפחות ושותפים.",
    guided: "התחל סיור",
  ...
  Français: {
    ...
    hero: "Bienvenue",
    subtitle: "Système numérique guidé pour les jeunes, superviseurs, familles et partenaires.",
...
const pathways: Record<PathwayKey, any> = {
  orientation: {
    ...
    need: "Youth need a safe, structured, and inspiring entry point before beginning work on the farm.",
    steps: [
    ...
    outcome: "Youth begin the program ready to work, learn, grow, and contribute.",
  },
  supervisor: {
    title: "Supervisor Dashboard",
    need: "Supervisors need a phone-friendly system to manage youth groups in real time.",
    steps: [
    ...
    outcome: "Supervisors become the operating leaders of the youth workforce ecosystem.",
  },
  tasks: {
    title: "Daily Farm Tasks",
    need: "Youth need daily assignments that connect real labor to real community outcomes.",
    steps: [
    ...
    outcome: "Daily farm work becomes measurable youth skill development.",
  },
  attendance: {
    title: "Attendance Tracking",
    need: "The program needs reliable daily records for participation, staffing, and reporting.",
    steps: [
    ...
    outcome: "Attendance becomes part of youth workforce discipline and program reporting.",
  },
  assessments: {
    title: "Youth Assessments",
    need: "Youth growth must be observed, documented, and encouraged throughout the summer.",
    steps: [
    ...
    outcome: "Youth leave with visible evidence of growth and workforce readiness.",
  },
  marketplace: {
    ...
    outcome: "Youth see how their work feeds people and supports the regional food ecosystem.",
  },
  parents: {
    ...
    outcome: "Families become connected stakeholders in youth growth.",
  },
  partners: {
    title: "Partner Support",
    need: "Partners need to see where their support fits inside the operating system.",
    steps: [
    ...
    outcome: "Partners become active contributors to a living community food ecosystem.",
  },
};
...
function InfoCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-white/15 bg-black/20 p-5">
      <div className="text-xs uppercase tracking-[0.25em] text-white/60 font-black">{label}</div>
      ...
      <section className="min-h-screen bg-[radial-gradient(circle_at_top_left,#6c7b42,transparent_30%),linear-gradient(135deg,#0e2213,#1f3d1d,#5b4a2d)] px-6 py-8">
        <header className="mx-auto flex max-w-7xl flex-col gap-5 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-4 rounded-[2rem] border border-white/15 bg-white/10 px-5 py-5 shadow-2xl backdrop-blur-md">
            ...
            <div>
              <div className="text-xs uppercase tracking-[0.35em] text-white/70">{copy.badge}</div>
              <div className="mt-2 text-2xl font-black leading-tight">{copy.title}</div>
            </div>
          </div>
          ...
          <>
            <section className="mx-auto mt-8 grid max-w-7xl gap-6 lg:grid-cols-2">
              <div className="rounded-[2.5rem] border border-white/15 bg-white/10 p-12 shadow-2xl backdrop-blur-md">
                <div className="text-sm uppercase tracking-[0.4em] text-white/60 font-black">Launch Ready</div>

                <h1 className="mt-8 text-6xl font-black leading-[0.95] md:text-8xl">{copy.hero}</h1>

                <p className="mt-8 max-w-2xl text-2xl leading-relaxed text-white/85">{copy.subtitle}</p>

                <div className="mt-10 grid gap-5 md:grid-cols-2">
                  <InfoCard label="Program Dates" value="June 8 – August 28, 2026" />
                  <InfoCard label="Schedule" value="Monday–Friday • 8AM–2PM" />
                  <InfoCard label="Orientation" value="June 5, 2026" />
                  <InfoCard label="Youth Capacity" value="50 Participants" />
                </div>
            ...
            <section className="mx-auto mt-10 max-w-7xl">
              <h2 className="mb-6 text-3xl font-black">{copy.choose}</h2>
              ...
              <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
                {(Object.keys(pathways) as PathwayKey[]).map((key) => (
                  ...
                  >
                    <div className="text-xs uppercase tracking-[0.3em] text-white/60">Workforce Pathway</div>
                    <div className="mt-4 text-3xl font-black">{pathways[key].title}</div>
                    <p className="mt-5 text-lg leading-relaxed text-white/80">{pathways[key].need}</p>
                  </button>
                ))}
              </div>
            </section>
            ...
            <div className="rounded-[2.5rem] border border-white/15 bg-white/10 p-10 shadow-2xl backdrop-blur-md">
              ...
              <div className="grid gap-8 xl:grid-cols-[1fr_420px]">
                <div>
                  ...
                  <h1 className="mt-5 text-6xl font-black leading-tight">{active.title}</h1>

                  <div className="mt-8 rounded-[2rem] bg-black/20 p-8">
                    <h2 className="text-2xl font-black">Need Being Met</h2>
                    <p className="mt-4 text-xl leading-relaxed text-white/85">{active.need}</p>
                  </div>
                </div>
                ...
                <div className="overflow-hidden rounded-[2rem] border border-white/15 bg-black/20">
                  ...
                  />
                  <div className="p-4 text-lg font-black">{active.title}</div>
                </div>
              </div>

              <div className="mt-8 grid gap-5">
                {active.steps.map((step: string, index: number) => (
                  <div key={index} className="rounded-[2rem] bg-white/10 p-7">
                    <div className="text-sm uppercase tracking-[0.3em] text-white/60">
                      Step {index + 1}
                    </div>
                    <div className="mt-4 text-xl leading-relaxed">{step}</div>
                  </div>
                ))}
              </div>

              <div className="mt-8 rounded-[2rem] bg-[#dce8c8] p-8 text-[#123018]">
                <div className="text-sm uppercase tracking-[0.3em]">Outcome</div>
                <div className="mt-4 text-2xl font-black leading-relaxed">{active.outcome}</div>
              </div>
            </div>

            <aside className="h-fit rounded-[2.5rem] border border-white/15 bg-black/20 p-8 shadow-2xl">
              <h2 className="text-3xl font-black">{copy.nextMoves}</h2>
              ...
              <div className="mt-6 grid gap-4">
                {[
                  ...
                  <div key={move} className="rounded-2xl bg-white/10 p-5 text-lg font-semibold">
                    {move}
                  </div>
                ))}
              </div>
              ...
              <div className="mt-8 rounded-[2rem] bg-white p-6 text-[#123018]">
                <div className="text-sm uppercase tracking-[0.3em]">Feedback & Reflection</div>
