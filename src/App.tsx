 ...
 * FULL COMPLETE INTEGRATED APP.TSX
 *
 * This file is intentionally self-contained for React + Vite.
 * It preserves the portal-first forest entrance and integrates:
 * registration, autofill-ready profiles, youth/supervisor morning baseline,
 * wellness-sensitive support routing, parent-safe progress, grower and
 * value-added registration, feedback, reports, crop planning, marketplace,
 * operations, live activity, Supabase-ready storage, and localStorage fallback.
 */
...
type ProfileType = "youth" | "supervisor" | "parent" | "grower" | "value_added" | "volunteer" | "partner" | "customer";
...
const PROFILE_KEY = "bff.ecosystem.profiles";
const YOUTH_KEY = "bff.ecosystem.youth";
const SUPERVISOR_KEY = "bff.ecosystem.supervisors";
const GROWER_KEY = "bff.ecosystem.growers";
const VALUE_ADDED_KEY = "bff.ecosystem.valueAdded";
const ATTENDANCE_KEY = "bff.ecosystem.attendance";
const ASSESSMENT_KEY = "bff.ecosystem.assessments";
const WELLNESS_KEY = "bff.ecosystem.wellness";
const FEEDBACK_KEY = "bff.ecosystem.feedback";
const CROP_KEY = "bff.ecosystem.cropPlans";
const ACTIVITY_KEY = "bff.ecosystem.activity";
...
function updateActivity(user: EcosystemUser | null, action: string) {
  ...
  safeWrite(ACTIVITY_KEY, [item, ...rows].slice(0, 40));
}
        ...
        <div className="max-w-4xl">
          <div className="rounded-[2.25rem] border border-white/10 bg-black/36 p-6 shadow-[0_40px_120px_rgba(0,0,0,.65)] backdrop-blur-xl md:p-9">
            ...
            <p className="mt-7 max-w-3xl text-lg leading-8 text-white/92 md:text-xl md:leading-9">
              Step through the forest gate. Follow the path. Discover how food, people, work, learning, wellness, and community move together.
            </p>
            <div className="mt-8 flex flex-wrap gap-4">
              <button onClick={() => setScreen("guest")} className="rounded-full bg-emerald-300 px-8 py-4 font-black text-black shadow-2xl transition hover:scale-105">Enter The Ecosystem</button>
              <button onClick={() => setScreen("registration")} className="rounded-full border border-white/20 bg-white/10 px-8 py-4 font-black text-white backdrop-blur-xl transition hover:bg-white/20">Register / Check In</button>
              <button onClick={() => setScreen("roles")} className="rounded-full border border-white/20 bg-black/35 px-8 py-4 font-black text-white backdrop-blur-xl transition hover:bg-white/20">Choose A Role</button>
            </div>
          </div>
        </div>
        <div className="rounded-[2.25rem] border border-white/10 bg-black/32 p-5 shadow-[0_40px_120px_rgba(0,0,0,.62)] backdrop-blur-xl">
          <div className="text-xs uppercase tracking-[0.35em] text-emerald-100/70">Live Foundation</div>
          <div className="mt-4 grid gap-3">
            {[
              "Registration feeds autofill.",
              "Morning check-ins establish baseline.",
              "Private wellness stays internal.",
              "Parent portal shows progress, not private reflections.",
              "Supervisor tools collect attendance, PPE, assessments, and alerts.",
            ].map((item) => (
...
function Guest({ setScreen }: { setScreen: (screen: Screen) => void }) {
  ...
  const steps = [
    ...
    { title: "The ecosystem reports impact.", text: "Attendance, PPE, assessments, wellness trends, crop plans, feedback, and parent progress become reports for leadership and funders.", image: IMG.partners },
  ];
  ...
  return (
    <Shell screen="guest" setScreen={setScreen} background={current.image}>
      <div className="grid gap-5 lg:grid-cols-[1fr_0.8fr]">
        <div className="rounded-[2rem] border border-white/10 bg-black/45 p-6 shadow-[0_35px_100px_rgba(0,0,0,.48)] backdrop-blur-2xl md:p-8">
          ...
          <div className="mt-6 rounded-2xl border border-white/10 bg-white/10 p-4 text-sm leading-6 text-white/80">
            Step {step + 1} of {steps.length}. Guided transitions should be slow enough for low readers to see the screen first, then receive the next layer.
          </div>
        </div>
        <div className="rounded-[2rem] border border-white/10 bg-black/45 p-5 backdrop-blur-2xl">
          ...
          <ActionGrid
            setScreen={setScreen}
            items={[
              { title: "Register", text: "Enter the system as youth, supervisor, grower, producer, parent, volunteer, partner, or customer.", screen: "registration" },
              { title: "Youth Pathway", text: "Protected youth check-in, wellness, attendance, and reflection.", screen: "youth" },
              { title: "Supervisor Tools", text: "Mobile workflow for attendance, PPE, assessment, and support alerts.", screen: "supervisor" },
...
function Account({ setScreen, signIn }: { setScreen: (screen: Screen) => void; signIn: (role: Role, name?: string) => EcosystemUser }) {
  ...
  return (
    <Shell screen="account" setScreen={setScreen} background={IMG.growArea2}>
...
function RegistrationHub({ setScreen, activeUser }: { setScreen: (screen: Screen) => void; activeUser: EcosystemUser | null }) {
  ...
  return (
    <Shell screen="registration" setScreen={setScreen} background={IMG.ecosystem}>
      <HeroCard eyebrow="Registration Hub" title="One intake. Many pathways." text="This is the shared profile layer. Youth, supervisors, parents, growers, value-added producers, volunteers, partners, and customers register here so future forms can autofill." image={IMG.growArea}>
        <div className="grid gap-4 md:grid-cols-3">
          <SelectField label="Registration type" value={profileType} onChange={(v) => setProfileType(v as ProfileType)} options={["youth", "supervisor", "parent", "grower", "value_added", "volunteer", "partner", "customer"]} />
...
function YouthScreen({ setScreen, activeUser }: { setScreen: (screen: Screen) => void; activeUser: EcosystemUser | null }) {
  return (
    <Shell screen="youth" setScreen={setScreen} background={IMG.youth}>
      <HeroCard eyebrow="Youth Workforce" title="Begin the day with awareness." text="Youth start with a simple baseline. This helps supervisors understand feelings, energy, needs, food readiness, and support without turning private reflection into parent-facing information." image={IMG.youth2}>
        <ActionGrid
          setScreen={setScreen}
          items={[
            { title: "Morning Wellness Check-In", text: "Mood, energy, sleep, breakfast, hope, belonging, trusted adult, confidence, and support needs.", screen: "wellness" },
            { title: "Daily Work Path", text: "Attendance, PPE, tasks, crop learning, safety, responsibility, and teamwork.", screen: "crop" },
...
function SupervisorScreen({ setScreen, activeUser }: { setScreen: (screen: Screen) => void; activeUser: EcosystemUser | null }) {
  ...
  const saveAttendance = async () => {
    const row: AttendanceRecord = { id: uuid(), participant_id: participantId || "unknown", supervisor_id: activeUser?.id, date: todayISO(), status, ppe_status: ppeStatus, notes };
  ...
  const saveAssessment = async () => {
    const row: AssessmentRecord = { id: uuid(), participant_id: participantId || "unknown", supervisor_id: activeUser?.id, date: todayISO(), safety, teamwork, communication, responsibility, notes };
  ...
  return (
    <Shell screen="supervisor" setScreen={setScreen} background={IMG.volunteers}>
      <HeroCard eyebrow="Supervisor Mobile Workflow" title="Supervisors protect, guide, observe, and record." text="Supervisors handle daily start, PPE, attendance, support observation, skill scoring, and safety routing. Each supervisor should work with no more than 15 youth." image={IMG.fencing}>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-4">
          {[["Safety", safety, setSafety], ["Teamwork", teamwork, setTeamwork], ["Communication", communication, setCommunication], ["Responsibility", responsibility, setResponsibility]].map(([label, value, setter]) => (
...
function WellnessScreen({ setScreen, activeUser }: { setScreen: (screen: Screen) => void; activeUser: EcosystemUser | null }) {
  ...
  const safetyFlag = hope <= 1 || trustedAdult <= 1 || /suicide|kill myself|hurt myself|overdose|drugs|homeless|unsafe|abuse/i.test(`${support} ${privateNote}`);
  ...
  return (
    <Shell screen="wellness" setScreen={setScreen} background={IMG.queens}>
      <HeroCard eyebrow="Wellness Baseline" title="Private check-in before work begins." text="These questions are not a diagnosis. They help identify immediate needs, emotional stress, food needs, belonging, and whether approved staff should follow up. Parent-facing summaries should not expose private youth answers." image={IMG.queens}>
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <SelectField label="Profile type" value={profileType} onChange={(v) => setProfileType(v as ProfileType)} options={["youth", "supervisor", "parent", "grower", "value_added", "volunteer", "partner", "customer"]} />
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-4">
          {[["Hope", hope, setHope], ["Belonging", belonging, setBelonging], ["Trusted adult", trustedAdult, setTrustedAdult], ["Confidence", confidence, setConfidence]].map(([label, value, setter]) => (
            <label key={label as string} className="rounded-2xl border border-white/10 bg-white/10 p-4">
              <div className="text-sm font-black">{label as string}</div>
              <input type="range" min={1} max={5} value={value as number} onChange={(e) => (setter as React.Dispatch<React.SetStateAction<number>>)(Number(e.target.value))} className="mt-4 w-full" />
              <div className="mt-2 text-2xl font-black">{value as number}/5</div>
...
function FeedbackScreen({ setScreen, activeUser }: { setScreen: (screen: Screen) => void; activeUser: EcosystemUser | null }) {
  ...
  return (
    <Shell screen="feedback" setScreen={setScreen} background={IMG.wkbn}>
...
function ParentScreen({ setScreen }: { setScreen: (screen: Screen) => void }) {
  return (
    <Shell screen="parent" setScreen={setScreen} background={IMG.growArea}>
      <HeroCard eyebrow="Parent / Guardian Portal" title="Progress without exposing private reflections." text="Parents should receive attendance, badges, skills, supervisor encouragement, family opportunities, and support notices when appropriate — not raw youth mental health responses." image={IMG.growArea}>
        <ActionGrid
          setScreen={setScreen}
          items={[
...
function GrowerScreen({ setScreen }: { setScreen: (screen: Screen) => void }) {
  return (
    <Shell screen="grower" setScreen={setScreen} background={IMG.csu}>
...
function ValueAddedScreen({ setScreen }: { setScreen: (screen: Screen) => void }) {
  return (
...
function CropScreen({ setScreen, activeUser }: { setScreen: (screen: Screen) => void; activeUser: EcosystemUser | null }) {
  ...
  const save = async () => {
    const row: CropPlan = { id: uuid(), crop, variety, zone, planted, grower_profile_id: activeUser?.id };
  ...
  return (
    <Shell screen="crop" setScreen={setScreen} background={IMG.growArea2}>
      <HeroCard eyebrow="Crop Planner" title="Plan what grows, where it grows, and when it started." text="This connects production, youth workforce tasks, grower coordination, market readiness, and reporting." image={IMG.seeds}>
...
function MarketplaceScreen({ setScreen }: { setScreen: (screen: Screen) => void }) {
  return (
    <Shell screen="marketplace" setScreen={setScreen} background={IMG.market}>
...
function ReportsScreen({ setScreen }: { setScreen: (screen: Screen) => void }) {
  const feedback = safeRead<FeedbackRecord[]>(FEEDBACK_KEY, []);
  const wellness = safeRead<WellnessCheckIn[]>(WELLNESS_KEY, []);
  ...
  return (
    <Shell screen="reports" setScreen={setScreen} background={IMG.partners}>
      <HeroCard eyebrow="Reports" title="Turn daily activity into leadership and funder evidence." text="Reports summarize registration, attendance, assessments, wellness trends, feedback, crop plans, market activity, and parent-safe progress." image={IMG.partners}>
        <StatGrid />
        <div className="mt-5 grid gap-4 md:grid-cols-2">
...
function OperationsScreen({ setScreen }: { setScreen: (screen: Screen) => void }) {
  return (
...
function PartnersScreen({ setScreen }: { setScreen: (screen: Screen) => void }) {
  return (
...
function SafetyScreen({ setScreen }: { setScreen: (screen: Screen) => void }) {
  return (
    <Shell screen="safety" setScreen={setScreen} background={IMG.fencing}>
      <HeroCard eyebrow="Safety + Support" title="Protect youth first." text="This screen is for approved staff. It separates support routing from public and parent-facing screens. It should connect to your real emergency protocol, staff contacts, incident forms, and mandated reporting procedures." image={IMG.fencing}>
        <div className="grid gap-4 md:grid-cols-3">
          {[
            "Immediate danger: contact emergency services and program leadership.",
            "Mental health concern: keep youth supervised, calm, and connected to an approved adult.",
            "Substance-use concern: document privately, route to staff, do not shame youth publicly.",
            "Parent communication: share safety/progress as appropriate, not private raw reflection.",
            "Staff access only: no random access to youth records.",
            "Follow-up: document action taken and next support step.",
...
function DataScreen({ setScreen }: { setScreen: (screen: Screen) => void }) {
  const rows = [
  ...
  return (
    <Shell screen="data" setScreen={setScreen} background={IMG.ecosystem}>
...
function RolesScreen({ setScreen, signIn }: { setScreen: (screen: Screen) => void; signIn: (role: Role, name?: string) => EcosystemUser }) {
  return (
    <Shell screen="roles" setScreen={setScreen} background={IMG.growArea2}>
...
function LockedScreen({ screen, setScreen }: { screen: Screen; setScreen: (screen: Screen) => void }) {
  return (
    <Shell screen={screen} setScreen={setScreen} background={IMG.forest}>
      <HeroCard eyebrow="Protected Access" title="This area is protected." text="This screen requires staff, admin, or approved family access. Enter by role before opening protected youth data or operational screens." image={IMG.forest}>
...
export default function App() {
  ...
  if (!canAccessScreen(activeUser, screen)) return <LockedScreen screen={screen} setScreen={guardedSetScreen} />;

  switch (screen) {
    case "portal":
      return <Portal setScreen={guardedSetScreen} />;
    case "guest":
      return <Guest setScreen={guardedSetScreen} />;
    case "registration":
      return <RegistrationHub setScreen={guardedSetScreen} activeUser={activeUser} />;
    case "account":
      return <Account setScreen={guardedSetScreen} signIn={signIn} />;
    case "roles":
      return <RolesScreen setScreen={guardedSetScreen} signIn={signIn} />;
    case "youth":
      return <YouthScreen setScreen={guardedSetScreen} activeUser={activeUser} />;
    case "supervisor":
      return <SupervisorScreen setScreen={guardedSetScreen} activeUser={activeUser} />;
    case "parent":
      return <ParentScreen setScreen={guardedSetScreen} />;
    case "grower":
      return <GrowerScreen setScreen={guardedSetScreen} />;
    case "valueAdded":
      return <ValueAddedScreen setScreen={guardedSetScreen} />;
    case "marketplace":
      return <MarketplaceScreen setScreen={guardedSetScreen} />;
    case "crop":
      return <CropScreen setScreen={guardedSetScreen} activeUser={activeUser} />;
    case "wellness":
      return <WellnessScreen setScreen={guardedSetScreen} activeUser={activeUser} />;
    case "feedback":
      return <FeedbackScreen setScreen={guardedSetScreen} activeUser={activeUser} />;
    case "reports":
      return <ReportsScreen setScreen={guardedSetScreen} />;
    case "operations":
      return <OperationsScreen setScreen={guardedSetScreen} />;
    case "partners":
      return <PartnersScreen setScreen={guardedSetScreen} />;
    case "safety":
      return <SafetyScreen setScreen={guardedSetScreen} />;
    case "data":
      return <DataScreen setScreen={guardedSetScreen} />;
    default:
      return (
        <Shell screen="portal" setScreen={guardedSetScreen} background={IMG.forest}>
